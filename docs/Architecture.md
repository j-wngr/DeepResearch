...
- **Entry points** (CLI verbs):
  - `research run "<question>"` — start a new run; derives the slug **up front from the raw question** (slugified title + short hash) and seeds the initial `ResearchState` (`question` + `slug`) before invoking the graph, so a `thread_id` exists before the first interrupt. The slug is opaque and never changes, even if clarify/approval reword the question. Runs an embed-model pre-flight check (`endpoints.check_embed_model`) before entering the graph and exits with a clear error if the configured model is not available on the Ollama host. Accepts `--skip-acquire` (env `SKIP_ACQUIRE_INTERRUPT=true`) to skip the manual-download interrupt and treat all blocked PDF fetches as unobtainable automatically, and `--bibliography-only` (env `BIBLIOGRAPHY_ONLY=true`) to disable all Tavily/PDF acquisition and use only sources already in the Bibliography (RAG-only mode).
  - `research resume <slug>` — reattach to a thread and answer the pending interrupt (editorial or acquisition). Also runs the embed-model pre-flight check on startup. Accepts the same `--skip-acquire` and `--bibliography-only` flags as `run`.
  ...
  - `research improve <slug>` — re-open the evaluation/refinement loop on a *finished* run (`user_approved=True`). Patches the checkpoint via `graph.update_state(as_node="writer")` to set `pending_handoff=True`, `user_approved=False`, `auto_round=0`, and mark all sub-topics dirty (`mark_all_dirty` from `nodes/evaluate.py`). The graph resumes from the writer node and proceeds through the normal evaluation hand-off interrupt. Research re-runs all sub-topics if the user requests changes; accepting without changes only replaces the report. Exits with an error if the slug has no checkpoint or the run is not yet finished.
...
- Source quality: `min_source_words` (default 150), `max_link_density` (default 0.5) — heuristic pre-filter thresholds. `quality_gate_enabled` (`QUALITY_GATE_ENABLED`, default `true`) — enable/disable the LLM source quality gate for newly fetched sources; the heuristic check still runs even when disabled.
- Acquisition flags: `skip_acquire_interrupt` (`SKIP_ACQUIRE_INTERRUPT`, default `false`) — treat blocked PDF fetches as unobtainable without pausing for a manual download. `bibliography_only` (`BIBLIOGRAPHY_ONLY`, default `false`) — disable all web searches and PDF fetches; research uses only sources already in the Bibliography pool.
